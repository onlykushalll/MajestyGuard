...
def _summary_stats_line(key: str, values: np.ndarray) -> str:
    median = float(np.median(values))
    p10 = float(np.percentile(values, 10))
    p90 = float(np.percentile(values, 90))
    mean = float(np.mean(values))
    return f"  {key:10s} median={median:.3f} p10={p10:.3f} p90={p90:.3f} mean={mean:.3f}"
...
def main() -> int:
    ...
    for key in [
        ...
        if values.size == 0:
            continue
        median = float(np.median(values))
        print(_summary_stats_line(key, values))
