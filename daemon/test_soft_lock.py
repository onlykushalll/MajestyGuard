...
def test_post_lock_idle_ignores_stale_timer_from_earlier_lock_period():
    """H15 regression guard.

    _post_lock_idle is scheduled via a 2s threading.Timer when LOCKED is
    entered. If the daemon leaves and re-enters LOCKED again before that
    timer fires, the stale timer must not be allowed to force the *new*,
    legitimate LOCKED period back to IDLE early.
    """
    daemon = _bare_daemon(State.IDLE)

    daemon._transition(State.LOCKED)
    stale_generation = daemon._transition_generation

    # Owner reacquired within the 2s window, then absence triggers a fresh,
    # legitimate LOCKED period before the original timer would have fired.
    daemon._transition(State.IDLE)
    daemon._transition(State.LOCKED)

    daemon._post_lock_idle(stale_generation)

    assert daemon.state == State.LOCKED, (
        "stale post-lock timer incorrectly overrode a fresh LOCKED period"
    )


def test_post_lock_idle_returns_to_idle_for_the_current_generation():
    """Companion to the stale-timer guard above: the intended behavior
    (returning to IDLE 2s after a LOCKED period with no re-entry) must still
    work for the case the guard is *not* supposed to block."""
    daemon = _bare_daemon(State.IDLE)

    daemon._transition(State.LOCKED)
    current_generation = daemon._transition_generation

    daemon._post_lock_idle(current_generation)
